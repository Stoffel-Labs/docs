...
This page describes the Stoffel 0.1.0 CLI from `Stoffel-Labs/StoffelVM`. The `stoffel` binary is a Cargo-like project tool; the lower-level `stoffel-run` helper executes compiled bytecode for local MPC runs.
...
```bash
stoffel test --local --runner /path/to/StoffelVM/target/debug/stoffel-run
```
...
- Build the runner with `cargo build -p stoffel-vm --bin stoffel-run` when local MPC commands report that `stoffel-run` is missing.
