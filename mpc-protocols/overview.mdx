...
If the decision depends on cost, read [Performance and circuit shaping](./performance-and-circuit-shaping). If the decision depends on config, bytecode, local runs, or network execution, read [How backend selection works](./implementation). If you need the application-level input, execution, and output flow, read [MPC Integration](../architecture/mpc).

## Building with an AI agent

If you are using an AI coding agent, give it the backend choice, secret/public input boundary, output boundary, and validation command before asking it to write code. The full prompt template lives in [Stoffel AI Agent Implementation](/developer-skills/stoffel-ai-agent-implementation).
