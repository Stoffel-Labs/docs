...
Use `ClientStore` when SDK clients supply their own private inputs through the client/coordinator protocol. Each client owns and submits its complete ordered input vector; an application server must not collect or proxy those plaintext values:
