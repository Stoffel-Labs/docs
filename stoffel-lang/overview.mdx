...
StoffelLang is the application language compiled by the `stoffellang` crate in the `StoffelVM` repository. It uses Python-like indentation, `def` functions, `var` bindings, static type annotations, lists, objects, closures, and explicit MPC share APIs.
