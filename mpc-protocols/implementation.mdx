...
AVSS share data can carry Feldman commitment material. StoffelLang exposes `AvssShare` helpers and `Share.get_commitment(...)` for commitment-oriented workflows such as threshold cryptography. Curve selection matters because scalar-field and group encodings must match the cryptographic protocol being built.
