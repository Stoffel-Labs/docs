...
description: "Run a multi-party Stoffel network with Docker Compose to understand coordinator, party, client, and backend topology."
---

Use the Docker Compose stacks in the Stoffel repository when you need to inspect the networked runtime shape: coordinator, MPC parties, client input processes, node RPC addresses, identity material, backend selection, and persistent preprocessing or local stores.

This is an operator/deployment-oriented workflow. For normal app development, start with local MPC through the `stoffel` CLI or Rust SDK.

## What the Docker stacks show

The Compose files are useful because they make the runtime topology explicit:
...
## How to read this as an app developer

The Docker stacks do not replace the app-facing CLI or SDK workflow. They show what your local `.execute_local().await?` or `stoffel run` workflow is abstracting away:
...
Use the stacks to understand topology, test packaged bytecode, or prepare operator runbooks. Keep first-project docs focused on `stoffel` CLI and Rust SDK surfaces.

## Current caveats

- These stacks are source-checkout workflows, not a managed production deployment product.
- Some coordinator paths are HoneyBadger/BLS12-381-specific while AVSS is exercised by a separate local-store stack.
- Compose networking uses fixed local subnets and checked-in local identity fixtures; production deployments need real identity, secret, network, and operations management.
