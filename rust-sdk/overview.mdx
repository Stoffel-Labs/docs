...
Stoffel::compile(...)
        ...
        │
        └── execute_local().await    local MPC test run
...
Use `MpcBackend::HoneyBadger` for the default field-arithmetic MPC backend. Use `MpcBackend::Avss { curve }` for AVSS workflows that need group/scalar shares or threshold elliptic-curve cryptography. See [MPC Protocols](../mpc-protocols/overview) for backend details.
...
The same builders accept `MpcBackend::Avss { curve: Curve::Bls12_381 }` for AVSS network/client experiments supported by the current off-chain client I/O path. Use curve-specific AVSS examples for threshold-signature workflows.
