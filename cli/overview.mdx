...
The `stoffel` CLI is the Cargo-like entry point for Stoffel 0.1.0 projects. It is built from `crates/stoffel-cli` in the `StoffelVM` repository.
...
Build the local runner in your StoffelVM checkout:
...
The Rust SDK path is the primary 0.1.0 application workflow. Python and Solidity templates are useful for exploration and integration planning, and should be verified against the StoffelVM repository before relying on generated code in a deployment.
...
Then pass `--runner /path/to/StoffelVM/target/debug/stoffel-run`.
