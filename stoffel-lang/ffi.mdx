...
description: "Foreign Function Interface for using the StoffelLang compiler from C, C++, Python, and other languages via C ABI."
---
...
The FFI exposes the StoffelLang compiler through a C ABI, allowing you to:

- Compile StoffelLang source code to bytecode
- Compile directly to VM bytecode format
...
Compiles directly to VM-compatible bytecode format (`.stflb` in the 0.1.0 CLI).
...
int main(int argc, char** argv) {
    ...
    // Write to file
    FILE* f = fopen("example.stflb", "wb");
    if (f) {
        fwrite(result->binary_data, 1, result->binary_len, f);
        fclose(f);
        printf("Written to example.stflb\n");
    }
