...
description: "An introduction to MPC - a cryptographic technique that allows multiple parties to jointly compute a function while keeping their inputs private."
---
...
- Used by protocols like Stoffel's HoneyBadgerMPC and AVSS backends, with backend-specific threshold rules
...
This is where Stoffel comes in: it packages MPC concepts into a compiler, VM, CLI, and SDK so developers can build with shares, parties, thresholds, and openings without implementing the protocols from scratch.
...
- **[Stoffel ecosystem](./ecosystem)** - Map the app-facing tools, bytecode path, VM runtime, and MPC backend
