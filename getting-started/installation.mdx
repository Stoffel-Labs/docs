...
<Note>
Using an AI coding agent? Start with the [agent install prompt](#install-stoffel-with-an-ai-coding-agent). It installs the `stoffel` CLI, adds Stoffel skills, connects docs access, and verifies the setup before the agent writes code.
</Note>
...
If you do not want your agent to run setup commands, run the same setup yourself:
...
Some agent harnesses have their own MCP setup commands or settings screens. See [Agent harness setup](/developer-skills/overview#agent-harness-setup) for harness-specific notes.
