...
<Note>
Using an AI coding agent? Install and verify the CLI first. Then use the optional agent setup below to install Stoffel skills and connect live docs access. The CLI docs remain canonical; the skills are task playbooks for agents.
</Note>
...
If you use AI coding agents, install the Stoffel skills so the agent has Stoffel-specific build, SDK, local MPC, and troubleshooting playbooks:

If you do not already have a terminal-based agent, install one first. For example:
...
Then paste this prompt into the agent:

```text
You are helping me build a Stoffel app.

First, install the Stoffel agent context for this workspace:

1. Confirm this terminal can run shell commands and that Node.js is available:
   node --version
   npm --version

2. Install the Stoffel developer skills:
   npx skills add https://docs.stoffelmpc.com --all

3. If this agent supports MCP, connect the live Stoffel docs:
   npx add-mcp --name stoffel-docs --transport http https://docs.stoffelmpc.com/mcp

4. Verify access by listing the installed Stoffel skills and searching the docs for `ClientStore` or `local MPC`.
...
You can also run the setup commands yourself:
...
After setup, ask the agent to verify access by listing the installed Stoffel skills and searching the docs for `ClientStore`. Then ask it to use [Stoffel App Getting Started](/developer-skills/stoffel-app-getting-started) for a first app or [Stoffel AI Agent Implementation](/developer-skills/stoffel-ai-agent-implementation) for a custom implementation task.
